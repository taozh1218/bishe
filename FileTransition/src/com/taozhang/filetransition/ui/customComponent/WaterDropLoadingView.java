...
import com.taozhang.filetransition.util.MyViewUtil;

/**
 * create by luoxiaoke on 2016/5/13 15:02. use for QQ健康 水滴加载view
 */
public class WaterDropLoadingView extends View {
	...
	private float mTriangleX1, mTriangleX2, mTriangleX3, mTriangleY1,
			mTriangleY2, mTriangleY3;
	// 控制三角形进度的高度值
	...
	public static final float MAX = 310; // 最大弧度
	/**
	 * 手机的dpi
	 */
	...
	public WaterDropLoadingView(Context context, AttributeSet attrs,
		...
		screenDPI = MyViewUtil.getScreenDPI((Activity)context);
		sDensityDpi = displayMetrics.densityDpi / screenDPI;

		// 圆心坐标赋值
		pointerX = Math.min(sWidth, sHeight) / 2;
		pointerY = Math.max(sWidth, sHeight) / 2;
		// 半径和三角形边长赋值
		mRaduis = pointerX / 2;
		pointerY = pointerY - mRaduis/2;
		// 赋值顶点坐标
		...
		mTriangleY2 = mTriangleY3 = (float) (pointerY - mRaduis
				* Math.sin(Math.PI / 3));
		...
		// 初始化速度范围的2个扇形外切矩形
		progressRectF = new RectF(pointerX - mRaduis + 8 * sDensityDpi,
				pointerY - mRaduis + 8 * sDensityDpi, pointerX + mRaduis - 8
						* sDensityDpi, pointerY + mRaduis - 8 * sDensityDpi);
		// 初始化起始角度
		startAngle = 90;
...
//		sweepAngle += 2;
		...
		if (sweepAngle < MAX) {
			...
			trianglePath1.moveTo(mTriangleX1, (float) (mTriangleY1 + mRaduis
					* Math.sin(Math.PI / 3) / 3));
			trianglePath1.lineTo(mTriangleX2 + 2
					* (mRaduis / 2 - triangleR / 2), mTriangleY2 + mRaduis / 2
					- triangleR / 2);
			trianglePath1.lineTo(mTriangleX3 - 2
					* (mRaduis / 2 - triangleR / 2), mTriangleY3 + mRaduis / 2
					- triangleR / 2);
			trianglePath1.close();
...
//	}
	...
	public void addSweepAngle(float sweepAngle) {
		
		this.sweepAngle += sweepAngle;
		this.startAngle -= sweepAngle / 2;
		this.invalidate();
	}
}
