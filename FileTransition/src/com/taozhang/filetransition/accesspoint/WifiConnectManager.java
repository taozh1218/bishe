...
import com.taozhang.filetransition.util.NetworkUtils;
...
public class WifiConnectManager extends WifiManagerWrap {
	...
	private void handleNetworkStateChanged(NetworkInfo networkInfo) {
		if (networkInfo.isConnected() && mWifiConnectListener != null) {
			String ssid = NetworkUtils.getCurrentSSID(App.getInstance());
			if (ssid != null) {
				if (ssid.contains(SSID_PREFIX)) // 2014-1-21 add
					mWifiConnectListener.onWifiConnected();
			} else {
				// ToastUtils
				// .showTextToast(MyApplication.getInstance(),
				// "鎮ㄥ綋鍓峎iFi瀛樺湪寮傚父锛岃閲嶆柊杩炴帴");
			}
		} else if (!networkInfo.isConnected()
				&& mWifiDisconnectListener != null) { // 2014-1-22 add
	...
	protected void handleWiFiStateChanged(int wifiState) {
		...
		case WifiManager.WIFI_STATE_ENABLED:
			mWifiManager.startScan();
			handler.postDelayed(runnable, Auto_Scan_Time); // scan WiFi each
															// Atuo_Scan_Time
			break;
	...
	public boolean connectToAccessPoint(String ssid, String bssid) {
		...
		// wifiConfig.preSharedKey = "\"" + DEFAULT_PASSWORD + "\"";//WiFi
		// encrypt
		...
		try {
			...
			field.set(wifiConfig,
					Enum.valueOf((Class<Enum>) field.getType(), "DHCP"));
		} catch (Exception e) {
			e.printStackTrace();
		}
